...
- **RESOLVED SITES (2026-08-09, build `0a6f5d1c`, pending test):** once the DLL
  ...
  - **UPDATE — those 3 alone did NOT fix it** (`patch_applied,115` confirmed they
    applied; deploy/build still broken). Root cause is the whole **`MapClass`
    content/occupancy subsystem at `0x568xxx`** (`AddContentAt`/`RemoveContentAt`
    and siblings), which building placement + occupancy go through, and which his
    74 patch **not at all**. It contains **13 `shl 9` cell-index sites + 21
    `cmp …,0x40000` bounds** (all verified: each `shl 9; add; cmp 0x40000; jge →
    return dummy off-map cell `0xABDC50`; else `mov [Items+idx*4]`). Left at 512
    the footprint registers in wrong slots AND every cell with index ≥ `0x40000`
    (Y ≥ 256 @1024) resolves to the dummy off-map cell → **can't deploy/build in
    >half the map**. Added all 34 (build `fbba6760`). The `cmp reg,0x40000` (81/7)
    quartet (`0x568498/4c4/8c8/8f4`) was individually confirmed to gate a
    `[MapClass+0x13C][idx]` read (not a 256 KB buffer FP). **This is the pattern
    for any remaining subsystem gaps: his curated base only covers the subsystems
    his manifests enumerated; each newly-exercised subsystem at >512 may need its
    `shl 9`/`cmp 0x40000` sites ported from the broad build's `kCellStrideSites`.**
  - **Maintenance note:** `curated/yr_map_512_patch_table.h` has a "do not
    hand-edit / generated by scripts/map512_emit_c_patch_table.py" header, but that
    generator/manifest is NOT in this repo — the vendored `.h` is the source of
    truth here, so hand-edits are correct until/unless the generator is vendored.
